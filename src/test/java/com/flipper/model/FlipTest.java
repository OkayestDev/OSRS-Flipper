...
public class FlipTest {
    ...
    public void testGetTotalProfitOnlyTakesAmountSoldIntoAccount() {
        Transaction buy = new Transaction(5, 1, 20, Instant.now(), true);
        Transaction sell = new Transaction(4, 1, 25, Instant.now(), false);
    ...
    public void testIsMarginCheckIsSetCorrectly() {
        Transaction buy = new Transaction(1, 1, 100, Instant.now(), true);
        Transaction sell = new Transaction(1, 1, 90, Instant.now(), false);
        Flip flip = new Flip(buy, sell);
